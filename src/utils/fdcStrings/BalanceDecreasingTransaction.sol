...
import {IBalanceDecreasingTransaction} from
    "dependencies/flare-periphery-0.0.22/src/coston2/IBalanceDecreasingTransaction.sol";
...
library FdcStrings {
    ...
    function toJsonString(IBalanceDecreasingTransaction.Response memory response)
        internal
        pure
    ...
    function toJsonString(IBalanceDecreasingTransaction.Proof memory proof) internal pure returns (string memory) {
        return string.concat(
            '{"merkleProof":', Base.toString(proof.merkleProof), ',"data":', toJsonString(proof.data), "}"
        );
    }

    function toJsonString(IBalanceDecreasingTransaction.RequestBody memory requestBody)
        internal
        pure
        returns (string memory)
    {
    ...
    function toJsonString(IBalanceDecreasingTransaction.ResponseBody memory responseBody)
        internal
        pure
        returns (string memory)
    {
